...
{ 
	...
	u32 addrx=0;//起始地址
	u32 endaddr=0;	//结束地址
	if(WriteAddr<STM32_FLASH_BASE||WriteAddr%4)return;	//非法地址(必须为4的倍数!!)
	FLASH_Unlock();									//解锁 
...
void STM32_FLASH_OneRead(u32 ReadAddr,u8 *pBuffer,u32 NumToRead)   	
{
	u32 i;
	for(i=0;i<NumToRead;i++)
	{
		pBuffer[i]=STM32_FLASH_ReadByte(ReadAddr);//读取1个字节.
		ReadAddr++;//偏移1个字节.	
	}
}
